...
For the bar chart of activity plotted by hours, we see a clear dip in activity during the evening hours. While we might suspect this would be the case based on a general knowledge of human activity, seeing it visualized helps us understand the magnitude of this shift. However, this only emphasizes the disparity between human data sources and automated data sources. We also don't know which of these systems have correctly set internal clocks.

For the pings by time over the course of the month, we see a significant drop-off in the final week of the month. This cannot be explained by a human behavior hypothesis, but instead points to an obscured limitation in the data collection process, in that a number of GPS units seemed to have stopped reporting to Skyhook during this week. This data does indicate how the reporting units in the city fluctuate with weekdays. This could be used as a predictive measure for fluctuating populations of the city on weekends (especially in the summer) but one must recognize the limitations in the dataset.
